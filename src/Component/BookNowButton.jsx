...
export default function BookNowButton({ course }) {
  const [open, setOpen] = useState(false);
  const { user, loading } = useAuth(); // ← useSession এর বদলে

  const isOwner = user?.id && course?.userId && user.id === course.userId;

  if (loading) {
    return (
      <div className="w-full h-14 bg-slate-100 dark:bg-slate-800 rounded-2xl animate-pulse mt-4" />
    );
  }
